    ...
    describe "Parsing Maryl language constructs" $ do
        ...
        context "Variables and numbers math" $ do
            ...
            it "((a ** b) % 30) / 2" $ do
                parseAST' "((a ** b) % 30) / 2;"
                    `shouldBe` Right
                        [ AstBinaryFunc
                            "/"
                            (AstBinaryFunc "%" (AstBinaryFunc "**" (AstVar "a") (AstVar "b")) (AstInt 30))
                            (AstInt 2)
                        ]
            ...
            it "(a + b++) ^ 30" $ do
                parseAST' "(a + b++) ^ 30;"
                    `shouldBe` Right
                        [ AstBinaryFunc
                            "^"
                            (AstBinaryFunc "+" (AstVar "a") (AstPostfixFunc "++" (AstVar "b")))
                            (AstInt 30)
                        ]
                ...
                parseAST' "(a-- + b++) ^ 30;"
                    `shouldBe` Right
                        [ AstBinaryFunc
                            ...
                            ( AstBinaryFunc
                                "+"
                                (AstPostfixFunc "--" (AstVar "a"))
                                (AstPostfixFunc "++" (AstVar "b"))
                            )
                            (AstInt 30)
                        ]
            it "(a + b++) ^ 30" $ do
            ...
            it "bool a = b == c" $ do
                parseAST' "bool a = b == c;"
                    `shouldBe` Right
                        [ AstDefineVar (Variable "a" Bool (AstBinaryFunc "==" (AstVar "b") (AstVar "c")))
                        ]
                ...
                parseAST' "int add(int a, int b) { return a + b; }"
                    `shouldBe` Right
                        [ AstDefineFunc
                            ( Function
                                "add"
                                [ AstDefineVar (Variable "a" Int AstVoid),
                                  AstDefineVar (Variable "b" Int AstVoid)
                                ]
                                [AstReturn (AstBinaryFunc "+" (AstVar "a") (AstVar "b"))]
                                Int
                            )
                        ]
                    ...
                    `shouldBe` Right
                        [ AstDefineFunc
                            ( Function
                                ...
                                [ AstReturn
                                    (AstBinaryFunc "==" (AstBinaryFunc "%" (AstVar "x") (AstInt 2)) (AstInt 0))
                                ]
                                Bool
                            )
                        ]
                ...
                parseAST' "char getFirstChar(string s) { return s[0]; }"
                    `shouldBe` Right
                        [ AstDefineFunc
                            ( Function
                                "getFirstChar"
                                [AstDefineVar (Variable "s" String AstVoid)]
                                [AstReturn (AstListElem "s" [AstInt 0])]
                                Char
                            )
                        ]
        ...
        context "If statement" $ do
            it "if (a < b) { return a; } else { return b; }" $ do
                parseAST' "if (a < b) { return a; } else { return b; }"
                    `shouldBe` Right
                        [ AstIf
                            (AstBinaryFunc "<" (AstVar "a") (AstVar "b"))
                            (AstBlock [AstReturn (AstVar "a")])
                            []
                            (Just (AstBlock [AstReturn (AstVar "b")]))
                        ]
            ...
            it "if (x == 0) { return 0; }" $ do
                parseAST' "if (x == 0) { return 0; }"
                    `shouldBe` Right
                        [ AstIf
                            (AstBinaryFunc "==" (AstVar "x") (AstInt 0))
                            (AstBlock [AstReturn (AstInt 0)])
                            []
                            Nothing
                        ]
            ...
            it "if (y > 10) { return y; } else if (y == 10) { return 0; }" $ do
                parseAST' "if (y > 10) { return y; } else if (y == 10) { return 0; }"
                    `shouldBe` Right
                        [ AstIf
                            (AstBinaryFunc ">" (AstVar "y") (AstInt 10))
                            (AstBlock [AstReturn (AstVar "y")])
                            [ AstIf
                                (AstBinaryFunc "==" (AstVar "y") (AstInt 10))
                                (AstBlock [AstReturn (AstInt 0)])
                                []
            ...
            it "if (flag) { x = 1; }" $ do
                parseAST' "if (flag) { x = 1; }"
                    `shouldBe` Right
                        [ AstIf
                            (AstVar "flag")
                            (AstBlock [AstBinaryFunc "=" (AstVar "x") (AstInt 1)])
                            []
                            Nothing
                        ]
            ...
            it "if (!boolean) { x--; }" $ do
                parseAST' "if (!boolean) { x--; }"
                    `shouldBe` Right
                        [ AstIf
                            (AstPrefixFunc "!" (AstVar "boolean"))
                            (AstBlock [AstPostfixFunc "--" (AstVar "x")])
                            []
                            Nothing
                        ]
            ...
            it "if (a && b) { return true; } else { return false; }" $ do
                parseAST' "if (a and b) { return true; } else { return false; }"
                    `shouldBe` Right
                        [ AstIf
                            (AstBinaryFunc "and" (AstVar "a") (AstVar "b"))
                            (AstBlock [AstReturn (AstBool True)])
                            []
                            (Just (AstBlock [AstReturn (AstBool False)]))
                        ]

            it
                "if (x > y) { return max; } else if (x < y) { return min; } else { return equal; }" $ do
                parseAST'
                    "if (x > y) { return max; } else if (x < y) { return min; } else { return equal; }"
                    `shouldBe` Right
                        [ AstIf
                            (AstBinaryFunc ">" (AstVar "x") (AstVar "y"))
                            (AstBlock [AstReturn (AstVar "max")])
                            [ AstIf
                                (AstBinaryFunc "<" (AstVar "x") (AstVar "y"))
                                (AstBlock [AstReturn (AstVar "min")])
                                []
                                Nothing
                            ]
                            (Just (AstBlock [AstReturn (AstVar "equal")]))
                        ]
            ...
            it "while (i < 10) { i = i + 1; }" $ do
                parseAST' "while (i < 10) { i = i + 1; }"
                    `shouldBe` Right
                        [ AstLoop
                            Nothing
                            (AstBinaryFunc "<" (AstVar "i") (AstInt 10))
                            ( AstBlock
                ...
                parseAST' "while (x != 0) { x = x - 1; }"
                    `shouldBe` Right
                        [ AstLoop
                            ...
                            (AstBinaryFunc "!=" (AstVar "x") (AstInt 0))
                            ( AstBlock
                                [AstBinaryFunc "=" (AstVar "x") (AstBinaryFunc "-" (AstVar "x") (AstInt 1))]
                            )
                        ]
            ...
            it "while (count < 100) { count += 10; }" $ do
                parseAST' "while (count < 100) { count += 10; }"
                    `shouldBe` Right
                        [ AstLoop
                            Nothing
                            (AstBinaryFunc "<" (AstVar "count") (AstInt 100))
                            (AstBlock [AstBinaryFunc "+=" (AstVar "count") (AstInt 10)])
                        ]
            ...
            it "while (n > 0) { n--; }" $ do
                parseAST' "while (n > 0) { n--; }"
                    `shouldBe` Right
                        [ AstLoop
                            Nothing
                            (AstBinaryFunc ">" (AstVar "n") (AstInt 0))
                            (AstBlock [AstPostfixFunc "--" (AstVar "n")])
                        ]
                ...
                parseAST' "while (flag) { x = x * 2; }"
                    `shouldBe` Right
                        [ AstLoop
                            ...
                            (AstVar "flag")
                            ( AstBlock
                                [AstBinaryFunc "=" (AstVar "x") (AstBinaryFunc "*" (AstVar "x") (AstInt 2))]
                            )
                        ]
        ...
        context "Operator parsing" $ do
            ...
            it "(a and b) or c" $ do
                parseAST' "(a and b) or c;"
                    `shouldBe` Right
                        [ AstBinaryFunc "or" (AstBinaryFunc "and" (AstVar "a") (AstVar "b")) (AstVar "c")
                        ]
                ...
                parseAST' "struct point {double x; double y; struct vector v;}"
                    `shouldBe` Right
                        [ AstDefineStruct
                            ( Structure
                                "point"
                                [ AstDefineVar (Variable "x" Double AstVoid),
                                  AstDefineVar (Variable "y" Double AstVoid),
                                  AstDefineVar (Variable "v" (Struct "vector") AstVoid)
                                ]
                            )
                        ]
                ...
                parseAST' "struct complex {string name; int value; const double rate;}"
                    `shouldBe` Right
                        [ AstDefineStruct
                            ( Structure
                                "complex"
                                [ AstDefineVar (Variable "name" String AstVoid),
                                  AstDefineVar (Variable "value" Int AstVoid),
                                  AstDefineVar (Variable "rate" (Const Double) AstVoid)
                                ]
                            )
                        ]
                    ...
                    `shouldBe` Right
                        [ AstDefineStruct
                            ( Structure
                                "complex"
                                [ AstDefineVar (Variable "name" String AstVoid),
                                  AstDefineVar (Variable "value" Double AstVoid)
                                ]
                            )
                        ]
                ...
                parseAST' "struct nested {struct complex c; int count;}"
                    `shouldBe` Right
                        [ AstDefineStruct
                            ( Structure
                                "nested"
                                [ AstDefineVar (Variable "c" (Struct "complex") AstVoid),
                                  AstDefineVar (Variable "count" Int AstVoid)
                                ]
                            )
                        ]
        ...
        context "Constant declaration" $ do
            ...
            it "constant missing value" $ do
                parseAST' "const int max;"
                    `shouldBe` Right [AstDefineVar (Variable "max" (Const Int) AstVoid)]
        ...
        context "Invalid syntax" $ do
            ...
            it "string greet(string name) { return \"Hello, \" + name; }" $ do
                isLeft (parseAST' "string greet(string name) { return \"Hello, \" + name; }")
                    `shouldBe` False
            ...
            it "Nested struct definition" $ do
                isLeft (parseAST' "struct outer {struct inner {int z;}; int y;}")
                    `shouldBe` True
