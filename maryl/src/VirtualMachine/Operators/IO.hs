...
import GHC.IO.Handle (Handle, hFlush, hGetContents, hGetLine, hPutChar, hPutStr)
import System.IO (IOMode (AppendMode, ReadMode, ReadWriteMode, WriteMode), hClose, openFile, stdout)
...
operatorPrint (S s : xs) = io $ putStr s >> hFlush stdout >> return xs
operatorPrint (C c : xs) = io $ putChar c >> hFlush stdout >> return xs
operatorPrint (val : xs) =
    io $ (putStr . show) val >> hFlush stdout >> return xs
-- operatorPrint (S s : xs) = io $ putStr s >> hFlush stdout >> return (N (fromIntegral (length s)) : xs)
-- operatorPrint (C c : xs) = io $ putChar c >> hFlush stdout >> return (N 1 : xs)
-- operatorPrint (val : xs) =
    -- io $ (putStr . show) val >> hFlush stdout >> return (N (fromIntegral (length (show val))) : xs)
